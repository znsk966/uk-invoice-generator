...
How the backend is put together, and why. Current as of Phase 5 (product &
service catalog; the backend is feature-complete and there is a working React
...
flowchart TD
    ...
    Trig["Integrity triggers<br/>invoices/immutability.py · products/integrity.py"]
...
| `db.py` | Engine, `SessionLocal`, declarative `Base`, the `get_session` request dependency, `is_unique_violation()` (maps a named-constraint race to a clean 409), and `check_db()` for `/health`. |
...
| `auth/service.py` | Register / login / logout and session lookup. Uniform 401 on any credential failure; unknown emails still run an argon2 verify (dummy hash) so timing doesn't reveal registration; concurrent duplicate registrations are 409 via a savepoint, never 500. |
...
| `invoices/models.py` | `Invoice` and `InvoiceLine` — inputs only, no computed money columns; JSONB `snapshot`; partial unique index on `number`; optional `InvoiceLine.product_id` catalog link. |
| `invoices/schemas.py` | Invoice schemas; every money field is `Decimal` with `allow_inf_nan=False`. |
| `invoices/service.py` | Draft CRUD, `resolve_lines` (applies the catalog rules to incoming lines), totals (`compute_invoice_totals`, `totals_from_snapshot`), the snapshot (v2), and the money-critical `issue_invoice` / `void_invoice`. |
...
| `products/models.py` | `Product` and the `product_kind` enum — owner-scoped catalog entry; immutable identity (code, description, kind, VAT rate), editable `unit_price`, `archived_at`. |
| `products/schemas.py` | Create/read schemas, the price-only `ProductPriceUpdate` (`extra="forbid"`), and `IMMUTABLE_FIELDS`. |
| `products/service.py` | `get_product_or_404` (owner-scoped) and `create_product` (savepoint → `409 product_code_taken` on a duplicate, even under a race). |
| `products/integrity.py` | The catalog trigger DDL (identity immutable, no deletes, linked lines must match their product), shared by migration and tests. |
| `products/router.py` | `/products` list (`include_archived`, `kind`) / create / get, price-only `PATCH`, `/archive`, `/unarchive`. No DELETE. |
...
| `POST /invoices/preview-totals` | rates + linked products | no | Unsaved editor edits. Lines are posted in the body; nothing is created. Catalog lines are resolved like a save (owner-scoped, VAT from the product) except that archived products are accepted. Lets the draft editor show live totals without autosaving on every keystroke. |
...
| `product_immutable` | 409 | `PATCH /products/{id}` naming an identity field (`code`, `description`, `kind`, `vat_rate_code`). Other unknown fields are `422 validation_failed`. |
| `product_code_taken` | 409 | Creating a product whose `code` this owner already uses. |
| `product_archived` | 409 | Newly adding an archived product to an invoice line (re-saving a draft that already links it is allowed). |
...
5. `b7c2e91f4a10` — the product catalog: `product` table, `invoice_line.product_id`, and the catalog triggers. Purely additive (no empty-DB guard); existing lines become ad-hoc (`product_id` NULL).

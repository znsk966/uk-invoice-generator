...
## Task 0 — Backend preconditions (the only backend changes allowed)

**0.1 — Fix `GET /invoices/{id}/totals` for non-draft invoices** (found during the 04A docs pass, noted in PR #4's description). Today it recomputes issued/void invoices at *today's* rates, so a post-issue rate change would make it disagree with the invoice's own snapshot. Fix: for `status != draft`, serve `snapshot.groups` and `snapshot.totals` verbatim — same source of truth as `GET /{id}`; recompute only for drafts. Update the endpoint docstring to state both behaviors. Test that proves it: issue an invoice, then INSERT a new `vat_rate` row (`standard`, different rate, `valid_from = today`) directly in the test, call `/totals`, and assert the response still equals the snapshot values — no recomputation.

**0.2 — Add `POST /api/v1/invoices/preview-totals`** — stateless totals preview for the editor. Body: `{ "lines": [{position, description, quantity, unit_price, vat_rate_code}], "on_date": "YYYY-MM-DD" (optional, default today) }`. Returns the same `InvoiceTotals` shape as `GET /invoices/{id}/totals`, computed via `rates_on` + `compute_totals`. Persists nothing. Reuses the existing line schema (string money). Two API tests: happy path matches `compute_totals`; pre-2011 date → 422 `validation_failed`.

0.2 exists so the editor can show live totals for **unsaved** edits without autosaving drafts on every keystroke.
